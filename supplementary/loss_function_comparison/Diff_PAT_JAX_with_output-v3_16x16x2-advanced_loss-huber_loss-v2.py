'''
advanced lossを確かめるために、lossを変えて実装
Huber lossで実装
M=512

slow_loss_func_for_verificationの方でだけ変えていたので、loss_funcに適用する
'''
...
def loss_func(all_phase, top_prop, bottom_prop, target_amp):
    ...
    #   loss_array += ( (err**2) )
      ...
      delta = 1
      err = est_amp - target_amp[i]
      is_small_err = jnp.absolute(err) < 1
      squared_loss = err**2 / 2
      linear_loss = jnp.absolute(err) - delta**2 / 2 

      huber_loss = jnp.where(is_small_err, squared_loss, linear_loss)
      loss_array += huber_loss
...
for x in range(3):
  ...
  for y in range(1000):
    ...
    # for ii in range(200):
    for ii in range(150):
      ### iterationごとの位相を保存するため ###
      iter_phase = get_params(opt_state)
      iter_phase_1d_arr = np.array( [iter_phase[0].T.ravel(), iter_phase[1].T.ravel()] ).ravel()
      iteration_phases.append(iter_phase_1d_arr)
      ### ###
    ...
    ### iterationごとの位相を保存するため ###
    iteration_phases.append(all_phase_1d_arr)
    iteration_phases = np.array(iteration_phases)

    np.savetxt(out_phase_p_path + 'iter_phases_p' + str(fn_num) + '_geo' + str(y) + '.csv', iteration_phases, delimiter=',')
    ### ###
