...
def prop_matrix(array_x, array_y, bottom_array_z, geo_arr, P0, k, r0, y):
  bottom_prop = []
...
@jit
def loss_func(all_phase, bottom_prop, target_amp):
    ...
    for i in range(point_num):
      ...
      point_Re = jnp.sum( bottom_point_Re )
      point_Im = jnp.sum( bottom_point_Im )
...
@jit
def step(step, opt_state, bottom_prop, target_amp):
  value, grads = jax.value_and_grad(loss_func)(get_params(opt_state), bottom_prop, target_amp)
  opt_state = opt_update(step, grads, opt_state)
  return value, opt_state

def slow_loss_func_for_verification(all_phase, bottom_prop, target_amp):
    bottom_phase = all_phase[0]
    bottom_phase_exp = jax.lax.complex(jnp.cos(bottom_phase), jnp.sin(bottom_phase))
    loss_array = 0
    for i in range(point_num):
...
for x in range(5):
  ...
  for y in range(1000): #for every sample points
    ...
    all_phase = ([init_bottom_phase])
    # Get array propetries
    bottom_prop = prop_matrix(array_x, array_y, bottom_array_z, geo_arr, P0, k, r0, y)
    ...
    # Optimize
    for ii in range(150):
      value, opt_state = step(ii, opt_state, bottom_prop, target_amp)

    # Output
    all_phase = get_params(opt_state)   
    calc_time = time.time() - start_time
    print("--- %s seconds ---" % calc_time)
    # loss_p = loss_func(all_phase, bottom_prop, target_amp)
    # loss_c = slow_loss_func_for_verification(all_phase, bottom_prop, target_amp)
    # print(loss_p)
    # print(loss_c)

    all_phase_1d_arr = np.array( [all_phase[0].T.ravel()] ).ravel()
